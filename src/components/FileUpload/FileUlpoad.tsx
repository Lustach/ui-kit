    ...
    <F.Form onDragEnter={handleDrag} onSubmit={(e) => e.preventDefault()}>
      ...
      <F.Label htmlFor="input-file-upload" className={dragActive ? 'drag-active' : ''}>
        ...
        <div>
          ...
          <F.FormButton buttonType="text" onClick={onButtonClick}>
            <span>Загрузите</span>
          </F.FormButton>
        </div>
      </F.Label>
      {dragActive && (
        <F.DragFileElement onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop} />
      )}
    </F.Form>
  )
}
